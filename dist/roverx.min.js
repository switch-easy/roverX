/*! roverX v2.0.0 | (c) 2026 switch-easy | 1200px Auto-Upscaler Active */
...
if (img.getAttribute('data-roverx-upgraded')) return;
const src = img.getAttribute('src') || '';
if (!src) return;
if (src.includes('ichef.bbci.co.uk')) {
const highRes = src.replace(/\/standard\/\d+\//g, '/standard/1024/').replace(/\/news\/\d+\//g, '/news/1024/');
if (highRes !== src) {
img.setAttribute('src', highRes);
img.setAttribute('data-roverx-upgraded', 'true');
return;
}
}
if (src.includes('i.guim.co.uk')) {
const highRes = src.replace(/width=\d+/g, 'width=1200');
img.setAttribute('src', highRes);
img.setAttribute('data-roverx-upgraded', 'true');
return;
}
if (src.includes('ytimg.com') || src.includes('youtube.com')) {
const highRes = src.replace(/\/(?:default|mqdefault|hqdefault)\.jpg/i, '/maxresdefault.jpg');
img.setAttribute('src', highRes);
img.setAttribute('data-roverx-upgraded', 'true');
return;
}
if (img.complete && img.naturalWidth > 0 && img.naturalWidth < 300) {
